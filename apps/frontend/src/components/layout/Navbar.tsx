  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-white/5">
        ...
        <div className="hidden md:flex items-center gap-0.5">
          {NAV.map(({ href, label, emoji }) => (
            <Link key={href} href={href}
              className={clsx('flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm transition-colors',
                pathname === href ? 'bg-brand-900/50 text-brand-300 border border-brand-700/30'
      ...
      <div className="md:hidden flex border-t border-white/5 overflow-x-auto">
        {NAV.map(({ href, label, emoji }) => (
          <Link key={href} href={href}
            className={clsx('flex-1 flex flex-col items-center gap-0.5 py-2 text-[10px] transition-colors min-w-[60px]',
